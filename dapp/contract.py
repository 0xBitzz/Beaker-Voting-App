...
from pyteal import *
from beaker import Application, ApplicationStateValue, AccountStateValue, Authorize, bare_external, external, internal, opt_in
...
class Asset(Application):
    ...
    def get_asset_bal(
        ...
        return Seq(
            (bal := AssetHolding.balance(account=account.address(), asset=self.asset_id)),
    ...
    # Vote
    @external(authorize=Authorize.only(Global.creator_address()))
    def setup(self, reg_begin: abi.Uint64, reg_end: abi.Uint64, vote_begin: abi.Uint64, vote_end: abi.Uint64):
        return Seq(
